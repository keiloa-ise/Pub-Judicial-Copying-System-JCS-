...
public sealed record CorrectCommand(
    Guid CopyRequestId, Guid? FormTemplateId, string FieldValuesJson, string SectionsJson, string DissentSectionsJson,
    string RebuttalSectionsJson, string Body);
    ...
    IUnitOfWork unitOfWork)
{
    ...
    public async Task CorrectAsync(CorrectCommand cmd, CancellationToken ct)
    {
        ...
        var rebuttalSectionsJson = RichText.SanitizeSectionsJson(cmd.RebuttalSectionsJson);
        // CorrectByReviewer requires UnderReview; the copy is NOT moved off that state here.
        request.CorrectByReviewer(cmd.FormTemplateId, cmd.FieldValuesJson, sectionsJson, dissentSectionsJson, rebuttalSectionsJson, cmd.Body, clock.UtcNow);
